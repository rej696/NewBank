...
public class NewBank implements NewBankQuery_I, Serializable {
  ...
  public String createCustomer(String username, String password) {
    if (isValidPassword(password)) {
      Customer customer = new Customer(
        SecurityUtilities.getSecurePassword(password)
      );
      customers.put(username, customer);
      return "Customer " + username + " Created";
    }
  ...
  private void addTestData() {
    Customer kim = new Customer(
      SecurityUtilities.getSecurePassword("Password123!")
    );
  ...
  public synchronized CustomerID checkLogInDetails(
    String userName,
    String password
  ) {
  ...
  public synchronized String processRequest(
    ...
    if (customers.containsKey(customer.getKey())) {
      ...
      if (
        stringInputs.length == 2 &&
        (stringInputs[1].equals("--help") || stringInputs[1].equals("-h"))
      ) {
        switch (command) {
          case "SHOWMYACCOUNTS":
        ...
        switch (command) {
          ...
          case "MOVE":
            {
              if (stringInputs.length > 3) {
                return moveFundsBetweenAccounts(
                  customer,
                  Double.parseDouble(stringInputs[1]),
                  stringInputs[2],
                  stringInputs[3],
                  true
                );
          ...
          case "OFFERLOAN":
            {
              if (stringInputs.length > 4) {
                return loanManager.offerLoan(
                  Double.parseDouble(stringInputs[1]),
                  stringInputs[2],
                  Integer.parseInt(stringInputs[3]),
                  Integer.parseInt(stringInputs[4])
                );
              }
            }
          case "SHOWMYOFFEREDLOANS":
          ...
          case "ACCEPTLOAN":
            {
              if (stringInputs.length > 2) {
                return loanManager.acceptLoan(
                  customer,
                  Integer.parseInt(stringInputs[1]),
                  stringInputs[2]
                );
              }
            }
          case "PAYBACKLOAN":
            {
              if (stringInputs.length > 1) {
                return loanManager.paybackLoan(
                  customer,
                  Integer.parseInt(stringInputs[1])
                );
              }
          ...
          case "PARTPAYBACKLOAN":
            {
              if (stringInputs.length > 2) {
                return loanManager.paybackLoanPartial(
                  customer,
                  Integer.parseInt(stringInputs[1]),
                  Double.parseDouble(stringInputs[2])
                );
              }
            }
          case "MAKEMONTHLYPAYMENT":
            {
              if (stringInputs.length > 1) {
                return loanManager.payMonthlyPayment(
                  customer,
                  Integer.parseInt(stringInputs[1])
                );
              }
  ...
  private String moveFundsBetweenAccounts(
    CustomerID customerID,
    double amount,
    String fromAccountNumber,
    String toAccountNumber,
    boolean accountsBelongToSameCustomer
  ) {
    Customer customer = this.getCustomer(customerID);
    Account account1 = customer.getAccount(fromAccountNumber);
    Account account2 = accountsBelongToSameCustomer
      ? customer.getAccount(toAccountNumber)
  ...
  private String getHelpLogout() {
    return "LOGOUT\t\t\t\t\t\t\tLogOut of the current Customer Account\n";
  }
}
